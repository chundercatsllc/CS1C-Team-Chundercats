...
Shape* dbConnection::createShapeObject(QString shapeType,int id,QPaintDevice *device,QString dimensions,QString penColor,QString penWidth,QString penStyle,QString penCapStyle,
    ...
    if(shapeType == "Line"){
        ...
        Qt::GlobalColor c = (getColor(textColor));
        Qt::AlignmentFlag f = (getFlag(textAlignment));
        QFont::Style s = (getFontStyle(fontStyle));
        QFont::Weight w = (getFontWeight(fontWeight));
        letsGetTexty->setPen(textPen);
        letsGetTexty->setFont(fontFamily,s,textPointSize.toInt(),w,c);
