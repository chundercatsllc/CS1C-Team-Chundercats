	...
	ui(new Ui::MainWindow)
{
    ...
    ui->menuStack->addWidget(ui->page);
...
void MainWindow::on_start_edit_button_clicked()
{
    ...
    ui->edit_vertex_num_spin->setEnabled(false);
}
...
void MainWindow::on_change_it_button_clicked()
{
    ...
    int x  = ui->edit_move_x_box->text().toInt();
    int y  = ui->edit_move_y_box->text().toInt();
    int v  = ui->edit_vertex_num_spin->value();
    ui->target->moveShape(id, v, x, y);
...
{
    if(ui->target->getShapeType(ui->shapeID_edit_spin->value() - 1) == Shape::ShapeType::Polygon ||
       ui->target->getShapeType(ui->shapeID_edit_spin->value() - 1) == Shape::ShapeType::Polyline)
        ui->edit_vertex_num_spin->setEnabled(true);
}
...
void MainWindow::on_reports_button_clicked()
{
    ...
    for(int i = 0; i < repThatVector.sizeOf(); i++)
        basicVector.push_back(repThatVector[i]);
